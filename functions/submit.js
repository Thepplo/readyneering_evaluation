...
export async function onRequestPost(context) {
  try {
    ...
    if (versionError || !versionRow) {
      return json({ error: 'Instrument version not found' }, 404);
    }
    ...
    if (items.length) {
      ...
      if (itemsError) {
        return json({ error: 'Failed to insert submission items' }, 500);
      }
    }
    ...
    if (scores.length) {
      ...
      if (scoresError) {
        return json({ error: 'Failed to insert submission scores' }, 500);
      }
    }

    return json({ ok: true, submission_id: submissionId }, 200);
  } catch (err) {
